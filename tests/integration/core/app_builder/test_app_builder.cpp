...
public:
  explicit DummyMainProcess(std::unique_ptr<IMainTask> task)
      : task_(std::move(task)) {}
...
public:
  explicit DummyHumanProcess(std::unique_ptr<IHumanTask> task)
      : task_(std::move(task)) {}
  int run() override { return 0; }
  void stop() override {}
...
public:
  explicit DummyBluetoothProcess(std::unique_ptr<IBluetoothTask> task)
      : task_(std::move(task)) {}
  int run() override { return 0; }
  void stop() override {}
...
public:
  explicit DummyBuzzerProcess(std::unique_ptr<IBuzzerTask> task)
      : task_(std::move(task)) {}
  int run() override { return 0; }
  void stop() override {}
...
inline auto make_app_injector() {
  ...
  return di::make_injector(di::bind<IMainTask>.to<MainTask>(),
...
std::unique_ptr<App> AppBuilder::build() {
  ...
  try {
    ...
    auto human_proc =
        std::make_unique<DummyHumanProcess>(std::move(human_task));
    auto bt_proc =
        std::make_unique<DummyBluetoothProcess>(std::move(bluetooth_task));
    auto buzzer_proc =
        std::make_unique<DummyBuzzerProcess>(std::move(buzzer_task));

    return std::make_unique<App>(std::move(main_proc), std::move(human_proc),
                                 std::move(bt_proc), std::move(buzzer_proc),
                                 std::move(logger));
  } catch (const std::exception &e) {
    if (logger) {
      logger->error(std::string("[AppBuilder::build] std::exception: ") +
                    e.what());
    } else {
      std::cerr << "[AppBuilder::build] std::exception: " << e.what()
                << std::endl;
    }
    throw;
  } catch (...) {
