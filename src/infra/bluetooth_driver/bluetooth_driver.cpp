...
BluetoothDriver::BluetoothDriver(
    std::shared_ptr<IFileLoader> device_loader,
    ...
    std::shared_ptr<IThreadSender> sender, std::shared_ptr<ILogger> logger)
    : device_loader_(std::move(device_loader)),
      distance_threshold_loader_(std::move(distance_threshold_loader)),
      scanner_(std::move(scanner)), pairer_(std::move(pairer)),
...
void BluetoothDriver::run() {
  ...
  if (!device_loader_ || !distance_threshold_loader_ || !scanner_ || !pairer_) {
    if (logger_)
      logger_->warn("BluetoothDriver dependencies not set");
    running_ = false;
    return;
  }
  ...
  for (const auto &dev : scanned) {
    if (std::find(registered.begin(), registered.end(), dev.mac) !=
            registered.end() &&
        dev.rssi >= threshold) {
...
void BluetoothDriver::stop() { running_ = false; }
