...
std::unique_ptr<App> AppBuilder::build() {
    ...
    try{
        auto main_process = injector.create<std::unique_ptr<IMainProcess>>();
        auto human_process = injector.create<std::unique_ptr<IHumanProcess>>();
        auto bluetooth_process = injector.create<std::unique_ptr<IBluetoothProcess>>();
        auto buzzer_process = injector.create<std::unique_ptr<IBuzzerProcess>>();
