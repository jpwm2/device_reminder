...
static std::string get_config_path() {
  ...
  return (fs::path(__FILE__)
              ...
              .parent_path()
              .parent_path() /
          "data/bluetooth_process.conf")
      .string();
}

TEST(BluetoothProcessIntegrationTest,
     正常系_RequestBluetoothScan_デバイス検出) {
  ...
  auto msg_for_send = std::make_shared<ProcessMessage>(
      ProcessMessageType::ResponseDevicePresence, std::vector<std::string>{});
  ...
  BluetoothProcess process(queue, receiver, dispatcher, sender, file_loader,
                           logger, watch_dog, handler, task);

  auto req_msg = std::make_shared<ProcessMessage>(
      ProcessMessageType::RequestBluetoothScan, std::vector<std::string>{});

  EXPECT_CALL(*codec, encode(_))
      .Times(2)
      .WillRepeatedly(Return(std::vector<uint8_t>{'a'}));
  ...
  std::vector<std::string> paired{"foo", "target"};
...
TEST(BluetoothProcessIntegrationTest, 異常系_BluetoothDriver例外) {
  ...
  EXPECT_CALL(*timer_service, start()).Times(1);
  EXPECT_CALL(*timer_service, stop()).Times(1);
  EXPECT_CALL(*driver, run())
      .WillOnce(::testing::Throw(BluetoothDriverError("run failed")));
  ...
  spd->flush();
  EXPECT_NE(oss->str().find("run failed"), std::string::npos);
}
